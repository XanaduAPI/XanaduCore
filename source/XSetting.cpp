...
void XSetting::_copy(const XSetting& _Setting) noexcept
{
	...
	{
		auto		vNodeCopy = XANADU_NEW XSettingPrivate();
		if(vNodeCopy)
		{
...
bool XSetting::load(const XString& _File) noexcept
{
	...
	auto		vHandle = Xanadu::wfopen(_File.data(), L"r");
	if(vHandle)
	{
		auto		vBuffer = XANADU_NEW char[XANADU_SIZE_MB];
		if(vBuffer)
		{
			while(0 == Xanadu::feof(vHandle))
			{
...
bool XSetting::save(const XString& _File) const noexcept
{
	...
	if(vHandle)
	{
		for (auto vNode = this->_Info; vNode; vNode = vNode->_Next)
		{
			...
			Xanadu::fwrite(vText.data(), 1, vText.length(), vHandle);
		}
