...
int32S XShell::run(const XString& _Shell, std::function<bool(const XString& _Output)> _Lambda) noexcept
{
	...
	if (::CreatePipe(&vRHandle, &vWHandle, &sa, 0))
	{
		...
		vShellCommand.remove(L" 2>&1");
		if(vShellCommand.empty())
		{
			return -1;
		}
		wchar_t 	vApplication[XANADU_PATH] = {0};
		auto		vCommandLine = XANADU_NEW wchar_t[static_cast<size_t>(vShellCommand.size()) + 100];
		if(vCommandLine)
		{
			Xanadu::wmemset(vCommandLine, 0, static_cast<size_t>(vShellCommand.size()) + 100);
			auto		vPos = XString::npos;
			if(vShellCommand[0] == L'\"')
			{
				vPos = vShellCommand.find(L'\"', 1U);
			...
			{
				vPos = vShellCommand.find(L' ');
			}
			if(vPos == XString::npos)
			{
				vPos = 0;
				Xanadu::wmemcpy(vApplication, vShellCommand.data(), vPos);
			}
			else
			{
				Xanadu::wmemcpy(vApplication, vShellCommand.data(), vPos);
			...
			Xanadu::wmemcpy(vCommandLine, vShellCommand.data() + vPos, static_cast<size_t>(vShellCommand.size() - vPos));
